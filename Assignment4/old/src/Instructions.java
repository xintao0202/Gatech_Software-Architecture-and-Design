...
private static void reset(CsvReader csv){
	for (Instructor ins:csv.instructors){
		ins.setTeaching(null);
	}
	for (Course c:csv.courses){
		c.availableSeats=0;
	}
	        ...
	        try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
	            ...
	            while ((line = br.readLine()) != null) {
	                ...
	                {       
	                 ...
	                 else if (instructor.currentTeaching!=null && (!instructor.currentTeaching.equals(course)) )
	                	 Output=Output+'\n'+line+'\n'+"# ERROR: instructor is already teaching a different course";
	                 else
	                 	{
	                	 instructor.setTeaching(course); 
	                	 course.addSeats(3);
	                ...
	                if(actions[0].equals("instructor_report")){
	                	Instructor instructor=getInstructor(actions[1], readCsv);
	                	 Output=Output+'\n'+line+'\n'+"# instructor, "+instructor.getName()+'\n'+"# "+instructor.currentTeaching.getID()+", "+instructor.currentTeaching.getTitle();
	                }
	                ...
	                if(actions[0].equals("request_course")){
	            		...
	            		else if(student.enrolledCourses.contains(course.getID()))
	            		...
	            		else if(course.availableSeats<1)
	            			Output=Output+'\n'+line+'\n'+"# not enrolled: no available seats";
	            		else {
	            			student.addEnrollment(course.getID());
	            			course.removeSeats(1);
	            			Output=Output+'\n'+line+'\n'+"# enrolled"; 
	            		}
	                ...
	                if(actions[0].equals("student_report")){
	                	...
	                	for (Grade g:student.academicRecord)
	                		if(g.comments!="")
	                			records=records+'\n'+g.course.getID()+", "+g.grade+", "+g.term.getSemester()+"_"+g.term.getYear()+", "+g.instructor.getID()+", "+g.comments;
	                		else
	                			records=records+'\n'+g.course.getID()+", "+g.grade+", "+g.term.getSemester()+"_"+g.term.getYear()+", "+g.instructor.getID();
